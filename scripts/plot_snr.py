...
def main(cfg: DictConfig):
    ...
    _, linear_diffusion = create_model_and_diffusion(**options)

    options["noise_schedule"] = "squaredcos_cap_v2"
    _, squaredcos_cap_v2_diffusion = create_model_and_diffusion(**options)

    # log the auc across timesteps to wandb
    linear_snr = linear_diffusion.base_diffusion.alphas_cumprod / (1 - linear_diffusion.base_diffusion.alphas_cumprod)
    squaredcos_cap_v2_snr = squaredcos_cap_v2_diffusion.base_diffusion.alphas_cumprod / (1 - squaredcos_cap_v2_diffusion.base_diffusion.alphas_cumprod)

    for t in range(1000):
        wandb.log({"linear_snr": linear_snr[t].item(),
                   "squaredcos_cap_v2_snr": squaredcos_cap_v2_snr[t].item()}, step=t)
